...
def get_captions_segments(video_id: str):
    raw = YouTubeTranscriptApi.get_transcript(video_id)  # list of {'text','start','duration'}
    segments = []
    for item in raw:
...
def transcribe_with_whisper(audio_path: str, model_size: str = "small"):
    if _WHISPER_IMPL == "faster":
        model = WhisperModel(model_size, device=WHISPER_DEVICE, compute_type="int8_float16" if not CUDA else "float16")
...
#pip install yt-dlp youtube-transcript-api gradio transformers torch sentencepiece
# choose one of these for Whisper:
#pip install "faster-whisper"       # recommended (fast, supports CPU/GPU)
# OR
#pip install "openai-whisper"      # original Whisper (slower on CPU)
