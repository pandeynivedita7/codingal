import cv2#For camera access and frame display.
import pycaw as pc # Corrected import for clarity.To control system audio volume.
import mediapipe as mp# For detecting hand and landmarks.
import math#For calculations like distance between fingers.

# Initialize MediaPipe Hands
mp_hands = mp.solutions.hands#mp_draw will be used to draw landmarks on hand.
mp_draw = mp.solutions.drawing_utils#Limits to 1 hand for simplicity.
...
interface = devices.GetSpeakers().Activate(
    pc.IAudioEndpointVolume._iid_, pc.CLSCTX_ALL, None)#Accesses the speaker device and gets the volume control interface.
...
min_vol = vol_range[0]#Retrieves min and max dB levels (e.g., -65.25 to 0.0).
max_vol = vol_range[1]
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            # Draw landmarks on the frame 21
            ...
            length = math.hypot(x2 - x1, y2 - y1)#Calculates Euclidean distance between thumb and index finger.
            ...
            volume.SetMasterVolumeLevel(target_vol_db, None)#Updates system master volume using pycaw.
