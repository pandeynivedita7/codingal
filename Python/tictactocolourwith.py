import random# used by AI to give any number between 1-10 0-8
from colorama import init, Fore, Style# representation of colour 
init(autoreset=True)

def display_board(board):# display 
    print()
    def colored(cell):# def funname(parameter):
        if cell == 'X':
...
def player_move(board, symbol):
    ...
    while move not in range(1, 10) or not board[move - 1].isdigit():
        try:# try write a code to check
            move = int(input("Enter your move (1-9): "))
            if move not in range(1, 10) or not board[move - 1].isdigit():
                print("Invalid move. Please try again.")
        except ValueError:# error except
...
def tic_tac_toe():
    ...
    while True:
        ...
        if play_again != 'yes':#! not operator 
