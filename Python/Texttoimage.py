...
from config import HF_API_KEY  # Your Hugging Face API key

# Define the API endpoint as a constant
API_URL = "https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-3-medium-diffusers" #This URL points to the model hosted on Hugging Face’s inference API.

def generate_image_from_text(prompt: str) -> Image.Image: #This function takes a text prompt as input and returns a PIL Image object.
    headers = {"Authorization": f"Bearer {HF_API_KEY}"}#Authorization Header: Uses your API key to authenticate with Hugging Face.
    payload = {"inputs": prompt}#Payload: Contains the user prompt as input for the model.
