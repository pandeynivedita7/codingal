-- Create the nomnom table if it does not exist Columns:
--NAME: Restaurant name.
--NEIGHBOURHOOD: Area or locality.
--CUISINE: Type of food served.
--REVIEW: Numeric rating.
--PRICE: Cost indicator.
--HEALTH: Health grade (A/B/etc.).
...
SELECT * --SELECT *: Selects all columns.
FROM nomnom;
-- Select distinct neighborhoods from the nomnom table
SELECT DISTINCT NEIGHBOURHOOD --Same concept: shows all unique cuisine types.
FROM nomnom;
