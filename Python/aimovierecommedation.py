import pandas as pd # array csv dataset excel file array pandas csv file excel 
from sklearn.feature_extraction.text import TfidfVectorizer #matrix text convert number w.r.t necessary matrix 
from sklearn.metrics.pairwise import cosine_similarity #similarty between movie
from textblob import TextBlob # sentimental analysis
from colorama import init, Fore # style colour font
import time # understand my sys time delay exiisting program
import sys
...
def load_data(file_path='imdb_top_1000.csv'):# Load and process csv excel file default dataset load datasetimdb moviedataset
    try:# part code that checks
        df = pd.read_csv(file_path)# read
        df['combined_features'] = df['Genre'].fillna('') + ' ' + df['Overview'].fillna('')# fiilna data preprocessing df preprcoseed
        return df#dataframe
    except FileNotFoundError:# error come here
...
tfidf = TfidfVectorizer(stop_words='english')# a,an,or not
tfidf_matrix = tfidf.fit_transform(movies_df['combined_features'])
cosine_sim = cosine_similarity(tfidf_matrix, tfidf_matrix)

# List all unique genres
def list_genres(df):
    return sorted(set(genre.strip() for sublist in df['Genre'].dropna().str.split(', ') for genre in sublist))#sorted list sublist 
...
def recommend_movies(genre=None, mood=None, rating=None, top_n=5):
    ...
    for idx, row in filtered_df.iterrows():
        ...
        if pd.isna(overview):# missing value
            continue#break
        polarity = TextBlob(overview).sentiment.polarity # >0 +ve =0 netral <0 -ve
...
def handle_ai(name):
    ...
    while True:
        ...
        if genre_input.isdigit() and 1 <= int(genre_input) <= len(genres):# len function of string give you no of chararter in a given string
    ...
    print(Fore.BLUE + "\nAnalyzing mood", end="", flush=True)#immediately
