...
from sklearn.datasets import fetch_openml # machine learning algorithm sklearn openml uswe to use open source dataset MNIST
from sklearn.linear_model import LogisticRegression# classification probabilities 0-9 classes 10 binary classification identify is a# digit or not preprocess
from sklearn.model_selection import train_test_split # dataset
from sklearn import metrics# score performance evaluating 
import matplotlib.pyplot as plt# visualization graphic 

# Load MNIST dataset from OpenML (you can also use sklearn's inbuilt datasets)
mnist = fetch_openml('mnist_784', version=1)# 28*28 784pixel 

# Data preprocessing labels data target perdicted 
X = mnist['data'] / 255.0  # Normalize the data (pixels between 0 and 1) image pixel 0-255 256 pixel image fraction floating int
...
accuracy = metrics.accuracy_score(y_test, y_pred)# precision recall f1
...
for i in range(5):  # You can change the range to display more images (e.g., 10 or more)
    plt.imshow(X_test.iloc[i].values.reshape(28, 28), cmap=plt.cm.binary)# test image series i .values numpy 784 reshape black white binary 0/1 colourmap
