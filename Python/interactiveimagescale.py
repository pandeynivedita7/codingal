import cv2
import numpy as np# array (width,height,channel)
...
def apply_color_filter(image, filter_type):# 
    ...
    if filter_type == "red_tint":
        ...
        filtered_image[:, :, 2] = cv2.add(filtered_image[:, :, 2], 50)#index value saturated addition 255 increase eg 100 o/p 150 230 255

    elif filter_type == "decrease_blue":
        # Decrease the intensity of the blue channel
        filtered_image[:, :, 0] = cv2.subtract(filtered_image[:, :, 0], 50)# sub decrease darkening 0 saturated sub 150 o/p 70 30 0
...
if image is None:
    ...
    while True:
        ...
        # Wait for key press
        key = cv2.waitKey(0) & 0xFF # interger dealy 114

        # Map key presses to filters
        if key == ord('r'): # ASCII Code ord() function
