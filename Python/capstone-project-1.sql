...
SELECT customer.cust_name,
    --SELECT tells SQL what columns you want to display.
    salesman.name,
    --Also select the name column from the Salesman table.
    salesman.city --Also select the city column from the Salesman table.
FROM Customer
    JOIN Salesman ON Customer.city = Salesman.city;
--JOIN means connect the Customer table with the Salesman table.
-- Linking customers to their salesmen
SELECT Customer.cust_name,
    --Only connect rows where both tables have the same city.
    Salesman.name --Match each customer with the salesman who works in the same city.
FROM Customer
...
SELECT Orders.ord_no,
    ...
    Orders.Salesman_id
FROM Orders
    JOIN Customer ON Orders.customer_id = Customer.customer_id --Orders.customer_id = Customer.customer_id
    JOIN Salesman ON Orders.Salesman_id = Salesman.Salesman_id --Orders.Salesman_id = Salesman.Salesman_id
